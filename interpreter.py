...
class TokenType:
    ...
    PLUS = "PLUS"
    MINUS = "MINUS"
    MUL = "MUL"  # Новий тип токену для операції множення
    DIV = "DIV"  # Новий тип токену для операції ділення
    LPAREN = "LPAREN"  # Новий тип токену для відкриваючої дужки
    RPAREN = "RPAREN"  # Новий тип токену для закриваючої дужки
...
class Lexer:
    ...
    def get_next_token(self):
        ...
        while self.current_char is not None:
            ...
            if self.current_char == "*":  # Розпізнання символу множення
                self.advance()
                return Token(TokenType.MUL, "*")

            if self.current_char == "/":  # Розпізнання символу ділення
                self.advance()
                return Token(TokenType.DIV, "/")

            if self.current_char == "(":  # Розпізнання відкриваючої дужки
                self.advance()
                return Token(TokenType.LPAREN, "(")

            if self.current_char == ")":  # Розпізнання закриваючої дужки
                self.advance()
...
class Parser:
    ...
    def term(self):
        """Парсер для 'term : factor ((MUL | DIV) factor)*' правил граматики."""
        node = self.factor()
    ...
    def expr(self):
        """Парсер для 'expr : term ((PLUS | MINUS) term)*' виразів."""
        node = self.term()
